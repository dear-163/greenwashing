...
st.sidebar.markdown("---")
st.sidebar.markdown("#### ⚡ **快速預覽**")
if st.sidebar.button("🧪 載入示範報告書數據 (Demo)", use_container_width=True):
...
with col_info1:
    if st.session_state["parsed_pages"]:
        ...
        <div style="background: rgba(16, 185, 129, 0.08); border: 1px dashed rgba(52, 211, 153, 0.4); border-radius: 10px; padding: 14px 18px; color: #94A3B8;">
            💡 <b>快速開始</b>：請在左側側邊欄上傳 PDF 格式的企業永續報告書；或直接點擊左側 <b>「🧪 載入示範報告書數據 (Demo)」</b> 立即檢驗系統功能與圖表！
        </div>
        """, unsafe_allow_html=True)
